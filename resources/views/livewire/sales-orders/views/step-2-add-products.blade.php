                                ...
                                <div class="overflow-x-auto">
                                    ...
                                    <x-list-table
                                        ...
                                        :rows="collect($products)->map(fn($stock) => [
                                            view('livewire.sales-orders.views.checkbox-product', ['stock' => $stock]),
                                            $product->name,
                                            $stock->stock_number,
                                            $stock->batch_number,
                                            $stock->expiration_date?->format('Y-m-d') ?? 'N/A',
                                            $stock->quantity - collect($cart)->where('stock_id', $stock->id)->sum('quantity'),
                                            number_format($stock->selling_price ?? $product->selling_price, 2),
                                            view('livewire.sales-orders.views.change-quantity-2', compact('stock', 'cart'))->render()
                                        ])"
                                    />
                                </div>
                            </div>
        ...
        <div class="space-y-8 md:px-6">
                    ...
                    <div class="overflow-x-auto">
                        <x-list-table
                            ...
                            :rows="collect($cart)->map(fn($item, $key) => [
                                view('livewire.sales-orders.views.name-code-and-expiry-date', ['item' => $item])->render(),
                                $item['stock_number'] ?? 'N/A',
                                '₱ ' . number_format($item['price'], 2),
                                view('livewire.sales-orders.views.change-quantity', compact('item', 'key'))->render(),
                                view('livewire.sales-orders.views.vat-tax', ['item' => $item])->render(),
                                '₱ ' . number_format($item['total'], 2),
                                view('livewire.sales-orders.views.delete-action', ['key' => $key])->render(),
                                '__model' => null,
                            ])"
                        />
                    </div>
            ...
            @else
                <div class="border border-gray-200 dark:border-gray-700 rounded-lg">
                    <div class="py-12 flex flex-col items-center justify-center text-center p-3">
