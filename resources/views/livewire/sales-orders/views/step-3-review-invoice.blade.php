@if ($currentStep === 3)
    <div class="flex items-center justify-between ">
        <div
            class="w-full justify-between dark:from-(--color-accent-4-dark) dark:to-gray-800 flex items-center rounded-lg">
            <div>
            ...
            <div class="flex items-center gap-4">
                <div class="relative inline-block">
                    <svg class="w-8 h-8 text-(--color-accent) dark:text-gray-400" xmlns="http://www.w3.org/2000/svg"
                    ...
                    @if (count($cart) > 0)
                        <span
                            class="absolute -top-1 -right-1.5 bg-(--color-accent) text-white dark:text-black text-[8px] font-bold w-5.5 h-5.5 flex items-center justify-center rounded-full shadow">
    ...
    <div class="space-y-2">
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <flux:select id="payment_terms" wire:model="payment_terms" :label="__('Payment Terms')"
                        size="md">
                        ...
                        <flux:select.option value="Partial">Partial Payment</flux:select.option>
                    </flux:select>
                ...
                {{-- <div>
                    <flux:select id="status" wire:model="status" :label="__('Status')" size="md">
                        <flux:select.option value="">Choose Status...</flux:select.option>
                        <flux:select.option value="draft">Draft / Quotation</flux:select.option>
                        <flux:select.option value="confirmed">Confirmed / Approved</flux:select.option>
                        <flux:select.option value="partially_delivered">Partially Delivered</flux:select.option>
                        <flux:select.option value="delivered">Delivered</flux:select.option>
                        <flux:select.option value="partially_invoiced">Partially Invoiced</flux:select.option>
                        <flux:select.option value="invoiced">Invoiced</flux:select.option>
                        <flux:select.option value="closed">Closed / Completed</flux:select.option>
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-600">
                <div class="flex items-center">
                    ...
                    <span
                        class="ml-2 px-2 py-1 text-xs bg-(--color-accent-muted) dark:bg-blue-900 text-(--color-accent) dark:text-blue-300 rounded-full">
            ...
            <div class="overflow-x-auto">
                <x-list-table :headers="['Product', 'Strength', 'Unit', 'Type', 'Price', 'Order Qty', 'VAT (12%)', 'Total']" :rows="collect($cart)->map(
                    fn($item) => [
                        $item['name'],
                        $item['strength'] ?? '',
                        $item['unit'] ?? '',
                        ...
                        $item['quantity'],
                        view('livewire.sales-orders.views.vat-tax', ['item' => $item])->render(),
                ...
                <div class="space-y-1 text-sm">
                    ...
                    <div class="flex justify-between py-1">
                        <div class="flex items-center gap-2">
                            ...
                            <flux:select wire:model.live="discount_type" :label="__('')" size="md"
                                value="">
                            ...
                            <div class="flex items-center gap-1">
                                @if ($discount_type === 'fixed')
                                    <flux:input type="number" wire:model.live="discount" placeholder="0.00"
                                        step="0.01" min="0" id="tax" :disabled="!['discount_type']">
                                ...
                                @elseif($discount_type === 'percentage')
                                    <flux:input type="number" wire:model.live="discount" placeholder="0.00"
                                        step="0.01" min="0" max="100" id="tax"
                                ...
                                @else
                                    <flux:input type="number" wire:model.live="discount" placeholder="0.00"
                                        step="0.01" min="0" id="tax" readonly></flux:input>
                                    <span class="text-sm text-zinc-500 dark:text-zinc-400">(%/₱)</span>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="flex justify-between items-center py-2">
                        ...
                        <span class= "font-medium text-gray-900 dark:text-white">₱
                            {{ number_format($totalVat, 2) }}</span>
                    </div>

                    <div
                        class="flex justify-between items-center py-3 border-t-2 border-gray-300 dark:border-gray-600">
                        <span class="text-md font-bold text-gray-900 dark:text-white">GRAND TOTAL:</span>
                        <span class="text-md font-bold text-(--color-accent) dark:text-blue-400">₱
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <flux:textarea :label="__('Terms & Conditions')" wire:model="terms_conditions" rows="3"
        ...
        <div
            class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pt-6 border-t border-gray-200 dark:border-gray-700 md:px-6">
            <flux:button variant="ghost" color="zinc" wire:click="backToStep2" icon="chevron-left">
                Back to Products
            </flux:button>

            <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <flux:button variant="primary" color="green" icon="printer"
                    wire:click="{{ $isEditing ? 'updateAndPrint' : 'print' }}">
                    {{ $isEditing ? 'Update & Print' : 'Create & Print' }}
                </flux:button>

                <flux:button variant="primary" color="blue"
                    wire:click="{{ $isEditing ? 'updateSalesOrder' : 'submitSalesOrder' }}">
    ...
    <div x-data="{
        ...
        <div x-show="showPreview" x-cloak class="fixed inset-0 z-50 flex"
            @open-print-dialog.window="showPreview = true" @pdf-loading-started.window="isLoading = true"
            @pdf-generation-complete.window="isLoading = false" x-transition:enter="ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">

            <div class="absolute inset-0 bg-gray-500 dark:bg-gray-800 opacity-75"></div>

            <div
                class="relative z-50 my-8 mx-auto p-5 w-11/12 max-w-6xl shadow-lg rounded-md bg-white max-h-[95vh] overflow-y-auto">
                ...
                <div class="mb-4 flex space-x-2">
                    <flux:button variant="primary" color="blue" icon="printer" onclick="printIframe()">Print
                    </flux:button>
                    <flux:button variant="primary" color="green" icon="document" wire:click="downloadPDF">Download
                        PDF</flux:button>
                </div>

                @if ($invoice)
                    <div x-show="isLoading" class="flex items-center justify-center h-64 text-gray-600">
                        <svg class="animate-spin h-6 w-6 mr-2 text-gray-500" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div x-show="!isLoading" class="h-[100vh] border border-gray-300 rounded-lg overflow-hidden">
                        <iframe id="pdfPreview" src="{{ route('sales-orders.stream-pdf', $invoice->id) }}"
                            width="100%" height="100%" class="w-full h-full" frameborder="0"
                            @load="isIframeLoaded = true" x-init="isIframeLoaded = false" x-show="isIframeLoaded">
                            Your browser does not support PDFs. Please download the PDF to view it.
                        </iframe>

                        <div x-show="!isIframeLoaded && !isLoading"
