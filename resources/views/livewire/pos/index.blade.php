...
<div>
    ...
    <div class="flex flex-col gap-4 rounded-xl">
        <h2>
            <span class="text-lg font-semibold text-gray-900 dark:text-gray-100">List of Invoices</span>
            <p>
                <span class="text-sm text-gray-500 dark:text-gray-400">Manage your invoices here.</span>
            </p>
        </h2>
        <div class="flex flex-wrap justify-between items-center gap-4">

            <div class="w-full sm:w-1/3">
                <input wire:model.live="search" type="search" placeholder="Search Invoices..."
        ...
        @if ($invoices->isEmpty())
            <div class="flex flex-col items-center justify-center p-8">
                ...
                <p class="mb-4 text-gray-500 dark:text-gray-400">No invoices found</p>
                @can('products.create')
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($invoices as $invoice)
                            <tr wire:key="product-{{ $invoice->id }}">
                                {{-- <td class="px-6 py-4 whitespace-nowrap">
                                    <img src="{{ $stock->getFirstMediaUrl('product-image') }}"
                                        alt="{{ $stock->product_name }}" class="w-12 h-12 rounded object-cover">
                                </td> --}}
                                <td class="px-6 py-4">{{ $invoice->product->product_code }}</td>
                                <td class="px-6 py-4">{{ $invoice->product_name }}</td>
                                <td class="px-6 py-4 text-center">{{ number_format($invoice->capital_price, 2) }}</td>
                                <td class="px-6 py-4 text-center">{{ number_format($invoice->selling_price, 2) }}</td>
                                <td class="px-6 py-4 text-center ">
                                    {{ $invoice->quantity }}
                                </td>
                                <td class="px-6 py-4 text-center ">
                                    {{ $invoice->formatted_manufactured_date }}
                                </td>
                                <td class="px-6 py-4 text-center ">
                                    {{ $invoice->formatted_expiration_date }}
                                </td>
