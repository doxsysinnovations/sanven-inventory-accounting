...
new class extends Component {
    ...
    {
        $this->pr = PurchaseRequest::with('items')->findOrFail($id);
        $this->request_type = $this->pr->request_type;
        $this->remarks = $this->pr->remarks;
    ...
    public function removeItem($index)
    {
        ...
        if (isset($item['id'])) {
            \App\Models\PurchaseRequestItem::find($item['id'])?->delete();
        }
    ...
    public function submit()
    {
        ...
        foreach ($this->items as $item) {
            if (isset($item['id'])) {
                ...
                \App\Models\PurchaseRequestItem::where('id', $item['id'])->update([
                    'product_name' => $item['product_name'],
                    'product_description' => $item['product_description'],
                    'quantity' => $item['quantity'],
                    'estimated_cost' => $item['estimated_cost'],
        ...
        session()->flash('message', 'Purchase Request updated!');
            ...
            <div class="space-y-2">
                ...
                <div @class([
                    ...
                    <div>
                        ...
                        <div class="text-xs">Edit & Submit</div>
                    </div>
                </div>
            </div>
            @if ($currentStep > 1)
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach ($items as $idx => $item)
                                            <tr>
                                                <td class="px-4 py-3">
                                                    <input wire:model="items.{{ $idx }}.product_name" type="text"
                                                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 px-2 py-1 text-sm text-gray-900 dark:text-gray-100" />
                                                </td>
                                                <td class="px-4 py-3">
                                                    <textarea wire:model="items.{{ $idx }}.product_description" rows="1"
                                                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 px-2 py-1 text-sm text-gray-900 dark:text-gray-100"></textarea>
                                                </td>
                                                <td class="px-4 py-3">
                                                    <input wire:model="items.{{ $idx }}.quantity" type="number" min="1"
                                                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 px-2 py-1 text-sm text-gray-900 dark:text-gray-100" />
                                                </td>
                                                <td class="px-4 py-3">
                                                    <input wire:model="items.{{ $idx }}.estimated_cost" type="number" min="0" step="0.01"
                                                        class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 px-2 py-1 text-sm text-gray-900 dark:text-gray-100" />
                                                </td>
                                                <td class="px-4 py-3">
