...
$classes = Flux::classes()
    ->add('h-10 lg:h-8 relative flex items-center gap-3 rounded-sm')
    ->add($square ? 'px-2.5!' : '')
    ...
    ->add(match ($variant) {
        'outline' => match ($accent) {
            true => [
                'data-current:text-white over:data-current:text-(--color-accent)',
                'data-current:bg-(--color-accent) dark:data-current:bg-white/[7%] data-current:border data-current:border-zinc-200 dark:data-current:border-transparent',
                'hover:text-(--color-accent) dark:hover:text-white dark:hover:bg-white/[7%] hover:bg-zinc-800/5 ',
